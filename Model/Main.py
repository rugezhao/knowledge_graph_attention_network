    ...
    *********************************************************
    ...
    if args.report == 1:
        ...
        for i, users_to_test in enumerate(users_to_test_list):
            ...
            # print(final_perf)
            args.logger.log_text({'final:performance': final_perf}, 0, print_to_stdout=True)
    ...
    *********************************************************
    ...
    for epoch in range(args.epoch):
        ...
        # retrieve latest embeddings 
            ...
            data_generator.ng_cluster(user_embedding, item_embedding, n_clusters = 25)
        ...
        *********************************************************
        ...
        if (epoch + 1) % show_step == 0:
            if args.verbose > 0 and epoch % args.verbose == 0:
                ...
                # print(perf_str)
                args.logger.log_text({'train:performance': perf_str},
                                    total_step_1 + total_step_2, cur_epoch=epoch, print_to_stdout=True)
        ...
        *********************************************************
        ...
        if args.verbose > 0:
            ...
            # print(perf_str)
            args.logger.log_text({'test:performance': perf_str},
                                        total_step_1 + total_step_2, cur_epoch=epoch,
                                        print_to_stdout=True)
            args.logger.log_scalars({'test:train_time': t2 - t1,
                                    'test:test_time': t3 - t2,
                                    'test:total_loss': float(loss),
                                    ...
                                    'test:hit_20': float(ret['hit_ratio'][0]),
                                    'test:hit_100':float(ret['hit_ratio'][-1]),
    ...
    # print(final_perf)
    args.logger.log_text({'test:performance': perf_str}, total_step_1 + total_step_2, cur_epoch=epoch,
                                        print_to_stdout=True)
